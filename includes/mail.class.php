...
class bsiMail 
{
	...
	function __construct() {
		...
		if($this->isSMTP == true){	
			...
			$this->emailFrom = $bsiCore->config['conf_portal_name']."<bsimarketingdept@gmail.com>";		
		}else{
			$this->emailFrom = $bsiCore->config['conf_portal_name']."<".$bsiCore->config['conf_portal_email'].">";
		}
	...
	public function loadEmailContent() {		
		$sql = mysqli_query($GLOBALS["___mysqli_ston"], "SELECT * FROM bsi_email_contents WHERE email_name = 'Confirmation Email'");
		$currentrow = mysqli_fetch_assoc($sql);	
