...
class bsiSearch
{
	...
	function __construct() {				
		...
		if($this->searchCode == self::SEARCH_CODE){
				$this->fullDateRange = $this->getDateRangeArray($this->mysqlCheckInDate, $this->mysqlCheckOutDate, $this->checkInTime, $this->checkOutTime);
	...
	private function setRequestParams() {		
		...
		$tmpVar = isset($_POST['pickuploc'])? $_POST['pickuploc'] : NULL; 
		$this->setMyParamValue($this->pickuploc, mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $tmpVar), NULL, true); 	
		
		$tmpVar = isset($_POST['dropoffloc'])? $_POST['dropoffloc'] : NULL; 
		$this->setMyParamValue($this->dropoffloc, mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $tmpVar), NULL, true); 	
	...
	private function setMySessionVars(){
		...
		if(isset($_SESSION['sv_pickup']))         unset($_SESSION['sv_pickup']);
		if(isset($_SESSION['sv_dropoff']))         unset($_SESSION['sv_dropoff']);
		...
		$_SESSION['sv_pickup']         = $this->pickuploc;	
		$_SESSION['sv_dropoff']         = $this->dropoffloc;	
	...
	private function getDateRangeArray($startDate, $endDate, $starttime, $endtime,  $nightAdjustment = true) {	
		global $bsiCore;
		...
		if ($time_to >= $time_from) { 
			if($nightAdjustment){
				if($bsiCore->config['conf_price_calculation_type']=='2'){
	...
	public function getAvailableCar(){
		...
		if($this->type != ""){
			$addquery .= ' and  car_type_id='.$this->type.'  ';
		}else{
			$addquery .= '';
		}	
		...
		OR 
		(ADDTIME(dropoff_datetime, '".$bsiCore->config['conf_interval_between_rent']."') BETWEEN '".$this->mysqlCheckInDate." ".$this->checkInTime."' AND '".$this->mysqlCheckOutDate." ".$this->checkOutTime."')) group by brd.car_id) as t1  on bcm.car_id=t1.car_id ) as p1 where total_car > booked_cars and car_id  in(select car_id from bsi_car_location where loc_id=".$this->pickuploc." and loc_type=1) 
and
car_id  in(select car_id from bsi_car_location where loc_id=".$this->dropoffloc." and loc_type=2) ".$addquery."   ";
		//echo $searchsql;
		//die;				   
