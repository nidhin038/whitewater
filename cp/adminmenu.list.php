...
if(isset($_GET['del'])){
	$del2=$_GET['del'];
   $r=mysqli_query($GLOBALS["___mysqli_ston"], "select * from bsi_adminmenu where parent_id=".mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_REQUEST['id']));
   if(@mysqli_num_rows($r)){
      $msg="<font face=verdana size=1 color=#ffffff><b>You Can't delete.agentmenus exists in this category.</b></font>";	  
   }else{
      $r=mysqli_query($GLOBALS["___mysqli_ston"], "delete from bsi_adminmenu where id=\"" .mysqli_real_escape_string($GLOBALS["___mysqli_ston"], $_REQUEST['id']). "\"");
...
$r=mysqli_query($GLOBALS["___mysqli_ston"], "select * from bsi_adminmenu where parent_id=0 order by ord");
	...
	echo"<tr bgcolor=#ffffff class=even>
			<td >";
				if(in_array($d['id'],$tids)):
					...
					$q=mysqli_query($GLOBALS["___mysqli_ston"], "select count(*) as total from bsi_adminmenu where  parent_id=" . $d['id']);
					$n=mysqli_num_rows($q);
			...
			echo "<td align=center>
				...
				<a class=lnk"; 
					...
					//echo" href='#'";
					echo"><u>".CAR_LIST_DELETE."</u>
				</a>
			</td>
		</tr>";
    	if(in_array($d['id'],$tids)):
			$rr=mysqli_query($GLOBALS["___mysqli_ston"], "select * from bsi_adminmenu where parent_id=" . $d['id'] . " order by ord");
			$k=0;
