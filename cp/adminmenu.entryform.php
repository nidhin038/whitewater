...
if(isset($_POST['Update'])){
   if(!$_POST['id']):
       $r=mysqli_query($GLOBALS["___mysqli_ston"], "insert into bsi_adminmenu(name,url,ord, parent_id) values ('". $_POST['name'] ."','".$_POST['url'] ."',". $_POST['ord'].",". $_POST['parent_id'].")");	
	  // echo "insert into bsi_adminmenu(name,url,ord, parent_id) values ('". $_POST['name'] ."','".$_POST['url'] ."',". $_POST['ord'].",". $_POST['parent_id'].")";
       ...
       $r=mysqli_query($GLOBALS["___mysqli_ston"], "update bsi_adminmenu set name='".$_POST['name']."',url='".$_POST['url']."',ord=".$_POST['ord']." where id=". $_POST['id']);
	   $id=$_POST['id'];
...
if(isset($_REQUEST['id'])){
   $r=mysqli_query($GLOBALS["___mysqli_ston"], "select * from  bsi_adminmenu where id=" . $_REQUEST['id']);
...
if(isset($_REQUEST['parent_id'])){
   $r=mysqli_query($GLOBALS["___mysqli_ston"], "select * from  bsi_adminmenu where id=" . $_REQUEST['parent_id']);
...
<tr><td></td><td ><input type="submit" value="<?php echo ADD_EDIT_CAR_SUBMIT;?>" name="Update" style="background:#e5f9bb; cursor:pointer; cursor:hand;" /></td></tr>
